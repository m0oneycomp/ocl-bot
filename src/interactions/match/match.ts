    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'host') {
            ...
            if (interaction.channel?.type === ChannelType.GuildText) {
                thread = await interaction.channel.threads.create({ name: `OCL Match - ${interaction.user.username}`, autoArchiveDuration: 60 });
            }
        ...
        if (sub === 'teams') {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#337def')
                .setThumbnail('https://i.imgur.com/f5LGesj.png'); // Utility Accent Logo
